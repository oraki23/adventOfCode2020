...
public class Day4 {
    ...
    public static void partOne(){
        int nbOfValidPassport = 0;
        List<String> lines = Arrays.asList(PUZZLE_INPUT.split("\n\n"));
        System.out.println("Number of passports: " + lines.size());

        for(String line : lines){
            Map<String, Boolean> paramsPresence = new HashMap<String,Boolean>();
            paramsPresence.put("byr", false);
            paramsPresence.put("iyr", false);
            paramsPresence.put("eyr", false);
            paramsPresence.put("hgt", false);
            paramsPresence.put("hcl", false);
            paramsPresence.put("ecl", false);
            paramsPresence.put("pid", false);
/*
            paramsPresence.put("cid", false);
*/

            for(Map.Entry<String, Boolean> entry : paramsPresence.entrySet()){
            ...
            boolean valid = true;
            for(Boolean bool : paramsPresence.values()){
                if(bool == false){
                    valid = false;
                    break;
                }
        ...
        System.out.println("Number of valid passports: " + nbOfValidPassport);
    }
...
*/
            ...
            for(Map.Entry<String, Boolean> entry : paramsPresence.entrySet()){
                ...
                if(index == -1){
                    ...
                    String pre = line.substring(index);

                    int x = Math.min(pre.indexOf(' ') >= 0 ? pre.indexOf(' ') : pre.length(), pre.indexOf('\n') >= 0 ? pre.indexOf('\n') : pre.length());

                    String value = pre.substring(4, x);

                    if(validatePass(entry.getKey(), value)){
                        entry.setValue(true);
                    }
                }
            }
    ...
    public static boolean validatePass(String key, String value){
        boolean valid = false;
        switch(key){
            case "byr":
                valid = (value.length() == 4 && Integer.parseInt(value) >= 1920 && Integer.parseInt(value) <= 2002) ? true : false;
                break;
            case "iyr":
                valid = (value.length() == 4 && Integer.parseInt(value) >= 2010 && Integer.parseInt(value) <= 2020) ? true : false;
                break;
            case "eyr":
                valid = (value.length() == 4 && Integer.parseInt(value) >= 2020 && Integer.parseInt(value) <= 2030) ? true : false;
                break;
            case "hgt":
                ...
                int val = Integer.parseInt(value.substring(0, value.indexOf(unit)));
                if(unit.equals("cm")){
                    valid = (val >= 150 && val <= 193) ? true : false;
                } else {
                    valid = (val >= 59 && val <= 76) ? true : false;
                }
                break;
            case "hcl":
                valid = value.matches("#([0-9a-f]){6}");
                break;
            case "ecl":
                valid = value.equals("amb") || value.equals("blu") || value.equals("brn") || value.equals("gry") || value.equals("grn") || value.equals("hzl") || value.equals("oth");
                break;
            case "pid":
                valid = value.matches("([0-9]){9}");
                break;
        }
    ...
    public static final String INVALID_PASS = "eyr:1972 cid:100\n" +
    ...
    public static final String VALID_PASS = "pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980\n" +
            "hcl:#623a2f\n" +
            "\n" +
    ...
    public static final String PUZZLE_TEST = "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd\n" +
    ...
    public static final String PUZZLE_INPUT = "eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241\n" +
            "\n" +
