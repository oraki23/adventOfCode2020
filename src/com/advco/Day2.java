...
public class Day2 {
    ...
    public static void partOne(){
        ...
        for(String s : input){
            int lowerBound = Integer.parseInt(s.substring(0, s.indexOf('-')));
            int higherBound = Integer.parseInt(s.substring(s.indexOf('-')+1, s.indexOf(' ')));
            char character = s.substring(s.indexOf(' ')+1, s.indexOf(':')).charAt(0);
            String pass = s.substring(s.indexOf(':')+2);

            System.out.println(lowerBound + "-" + higherBound + " " + character + ": " + pass);

            int numberOfSearchedCharacter = 0;
            for(int i = 0; i < pass.length(); i++){
                if(pass.charAt(i) == character){
                    numberOfSearchedCharacter++;
                }
            }

            if(numberOfSearchedCharacter >= lowerBound && numberOfSearchedCharacter <= higherBound) {
                nbOfValidPass++;
            }
        ...
        System.out.println("Nb of valid pass: " + nbOfValidPass);
    }
    ...
    public static void partTwo(){
        ...
        for(String s : input){
            int position1 = Integer.parseInt(s.substring(0, s.indexOf('-')));
            int position2 = Integer.parseInt(s.substring(s.indexOf('-')+1, s.indexOf(' ')));
            char character = s.substring(s.indexOf(' ')+1, s.indexOf(':')).charAt(0);
            String pass = s.substring(s.indexOf(':')+2);

            System.out.println(position1 + "-" + position2 + " " + character + ": " + pass);

            for(int i = 0; i < pass.length(); i++){
                if(pass.charAt(position1-1) == character && pass.charAt(position2-1) == character){
                    break;
                } else if(pass.charAt(position1-1) == character || pass.charAt(position2-1) == character){
                    nbOfValidPass++;
                    break;
                }
            }
