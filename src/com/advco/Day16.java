...
public class Day16 {
    ...
    public static void partOne(){
        String[] entries = PUZZLE_INPUT.split("\n\n");

        String[] categories = entries[0].split("\n");
        String yourTicket = entries[1].split("\n")[1];
        String[] nearbyTickets = Arrays.copyOfRange(entries[2].split("\n"),1,entries[2].split("\n").length);

        System.out.println("Categories: ");
        HashMap<String, Integer[]> validNotes = new HashMap<>();
        for(int i = 0; i < categories.length; i++){
            System.out.println(categories[i]);
            String name = categories[i].substring(0, categories[i].indexOf(":"));
            String minMax1 = categories[i].substring(categories[i].indexOf(":")+2, categories[i].indexOf(" or "));
            String minMax2 = categories[i].substring(categories[i].indexOf(" or ")+4);

            int min1 = Integer.parseInt(minMax1.substring(0, minMax1.indexOf("-")));
            int max1 = Integer.parseInt(minMax1.substring(minMax1.indexOf("-")+1));
            int min2 = Integer.parseInt(minMax2.substring(0, minMax2.indexOf("-")));
            int max2 = Integer.parseInt(minMax2.substring(minMax2.indexOf("-")+1));

            validNotes.put(name, new Integer[]{ min1, max1, min2, max2 });
        }
        System.out.println("yourTicket: " + yourTicket);
        System.out.println();
        ...
        for(int i = 0; i < nearbyTickets.length; i++){
            ...
            String[] numbersOnTicket = nearbyTickets[i].split(",");
            for(int j = 0; j < numbersOnTicket.length; j++){
                int number = Integer.parseInt(numbersOnTicket[j]);
                boolean valid = false;
                for(int x = 0; x < validNotes.size(); x++){
                    if((number >= ((Integer[])validNotes.values().toArray()[x])[0] && number <= ((Integer[])validNotes.values().toArray()[x])[1]) ||
                            (number >= ((Integer[])validNotes.values().toArray()[x])[2] && number <= ((Integer[])validNotes.values().toArray()[x])[3])){
                        valid = true;
                    }
                }

                if(!valid){
                    invalidValuesSum = invalidValuesSum + number;
                }
            }
        }
        System.out.println("invalidValuesSum: " + invalidValuesSum);
    ...
    public static void partTwo(){
        ...
        System.out.println("Categories: ");
        List<String> validNotesName = new ArrayList<>();
        List<Integer[]> validNotesArray = new ArrayList<>();
        ...
        for(int i = 0; i < categories.length; i++){
            ...
            validNotesName.add(name);
            validNotesArray.add(new Integer[]{ min1, max1, min2, max2 });
        }
        System.out.println();
        System.out.println("yourTicket: " + yourTicket);
        System.out.println();
        ...
        for(int i = 0; i < nearbyTickets.length; i++){
            ...
            for(int j = 0; j < numbersOnTicket.length; j++){
                int number = Integer.parseInt(numbersOnTicket[j]);
                boolean valid = false;
                for(int x = 0; x < validNotesArray.size(); x++){
                    if((number >= ((Integer[])validNotesArray.toArray()[x])[0] && number <= ((Integer[])validNotesArray.toArray()[x])[1]) ||
                            (number >= ((Integer[])validNotesArray.toArray()[x])[2] && number <= ((Integer[])validNotesArray.toArray()[x])[3])){
                        valid = true;
                    }
                }
                if(!valid){
            ...
            if(invalidValuesSum == 0){
                validNearbyTickets.add(nearbyTickets[i]);
            }

        }

        String[] categoriesOrder = new String[validNotesName.size()];

        while(Arrays.asList(categoriesOrder).contains(null)){
            for(int i = 0; i < categoriesOrder.length; i++){
                int nbOfValidCategories = 0;
                int indexOfValidCat = 0;
                for(int j = 0; j < validNotesName.size(); j++){
                    boolean valid = true;
                    //Regarder la premiere position de chaque ticket, et si tous les tickets sont valides, alors on a trouvé la catégorie
                    for(String validTicket : validNearbyTickets){
                        int valid2 = 0;
                        String[] numbersOnTicket = validTicket.split(",");
                        int number = Integer.parseInt(numbersOnTicket[i]);
                        if((number >= ((Integer[])validNotesArray.toArray()[j])[0] && number <= ((Integer[])validNotesArray.toArray()[j])[1]) ||
                                (number >= ((Integer[])validNotesArray.toArray()[j])[2] && number <= ((Integer[])validNotesArray.toArray()[j])[3])){
                            valid2++;
                        }
                ...
                if(nbOfValidCategories == 1){
                    categoriesOrder[i] = (String)validNotesName.toArray()[indexOfValidCat];
                    validNotesName.remove(indexOfValidCat);
                    validNotesArray.remove(indexOfValidCat);
                }
            }
            System.out.println("categoriesOrder: " + Arrays.asList(categoriesOrder));

        }

        BigInteger multiplication = BigInteger.ONE;
        for(int i = 0; i < categoriesOrder.length; i++){
            if(categoriesOrder[i] != null && categoriesOrder[i].contains("departure")){
                String[] numbersOnTicket = yourTicket.split(",");
                int number = Integer.parseInt(numbersOnTicket[i]);
    ...
    public static final String PUZZLE_TEST = "class: 1-3 or 5-7\n" +
            "row: 6-11 or 33-44\n" +
            "seat: 13-40 or 45-50\n" +
            "\n" +
            "your ticket:\n" +
            ...
            "55,2,20\n" +
            "38,6,12";
    public static final String PUZZLE_TEST_2 = "class: 0-1 or 4-19\n" +
            "row: 0-5 or 8-19\n" +
            "seat: 0-13 or 16-19\n" +
            "\n" +
            "your ticket:\n" +
            ...
            "15,1,5\n" +
            "5,14,9";
    public static final String PUZZLE_INPUT = "departure location: 43-237 or 251-961\n" +
